...
st.caption(
    ...
    "Ingestion unique, append-only, même pipeline analytique."
)
    ...
    with left:
        ...
        if submit_manual:
            try:
                ...
                st.info(f"Entry saved and analyzed ({n} entry recomputed).")
...
with tab_csv:
    ...
    st.caption(
        "Import d’un CSV. Chaque ligne est ingérée individuellement "
        "via le même pipeline que la saisie manuelle."
    )
    ...
    if uploaded_file:
        ...
        st.info(
            f"{len(reader.fieldnames)} columns detected: "
...
st.caption(
    ...
    "based on current YAML configuration."
)
